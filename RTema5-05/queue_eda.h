...
public:
	...
	void invierte() {
		...
		act = prim;
		sig = prim->sig;
		//Primer caso, prim apunta a nullptr
		while (act != ult) {
			if (ant == nullptr) {
				act->sig = nullptr;
			}
			else {
				act->sig = ant;		//Apuntamos el siguiente al anterior para invertir
			}
			ant = act;			//Actualizamos el anterior al actual
			act = sig;			//Avanzamos el actual al siguiente
			sig = sig->sig;		//Avanzamos el siguiente
		}
		act->sig = ant;
		ult = prim;		//Cambiamos el ultimo por el primero para invertir
		prim = act;		//Cambiamos el prim por act que deberia ser el ultimo
	}
