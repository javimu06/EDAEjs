...
using listaProfes = unordered_map<Profesor, listaAlumnos>;
using listaAlumnos = map<Alumno, Puntos>;

class AutoEscuela {
private:
	listaProfes ListaProfes_;
public:
	AutoEscuela() {};
	void alta(Alumno a, Profesor p) {
		//Si la autoescuela no tiene a ese alumno matriculado
		if (ListaProfes_.find(a) == 0) {
			ListaProfes_[p][a] = 0;		//Lo anadimos y ponemos sus puntos a 0
		}
		else {
